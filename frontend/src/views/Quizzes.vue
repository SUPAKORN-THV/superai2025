      ...
      >
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex space-x-2">
            <router-link
              v-if="isQuizCreator(quiz)"
              :to="`/edit-quiz/${quiz._id}`"
              class="bg-gray-600 text-white px-3 py-2 rounded-md hover:bg-gray-700 transition duration-300 text-sm font-medium"
            >
              Edit
            </router-link>
            <button
              ...
              class="bg-red-600 text-white px-3 py-2 rounded-md hover:bg-red-700 transition duration-300 text-sm font-medium"
            >
              Delete
            </button>
            <router-link
...
const isQuizCreator = (quiz) => {
  if (!quiz.createdBy || !authStore.user) return false
  
  const quizCreatorId = typeof quiz.createdBy === 'object' ? quiz.createdBy._id : quiz.createdBy
  const currentUserId = authStore.user._id || authStore.user.id
  
  return quizCreatorId === currentUserId || authStore.isAdmin
}
