          ...
          <div v-else-if="currentQuestion.type === 'code'">
            <div class="mb-4">
              ...
              <textarea
                v-model="currentAnswer"
                rows="12"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm bg-gray-50"
...
const loadCurrentAnswer = () => {
  ...
  if (answer && answer.answer) {
    ...
    if (currentQuestion.value.type === 'multiple-choice') {
      currentAnswer.value = []
    } else if (currentQuestion.value.type === 'code' && currentQuestion.value.codeTemplate) {
      currentAnswer.value = currentQuestion.value.codeTemplate
    } else {
      currentAnswer.value = ''
    }
  }
